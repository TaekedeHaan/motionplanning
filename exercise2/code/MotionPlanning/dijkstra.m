...
% goalIdx: a 2D index of the goal vertex 
...
% from the goal.
costs(goalIdx(1), goalIdx(2)) = 0; % dilatedMap(goalIdx(1), goalIdx(2))
...
% cost: the distance to the goal
% heap: contains lowest cost coordinate
...
nodes{1} = [node(1) + 1, node(2)];
nodes{2} = [node(1), node(2) - 1];
nodes{3} = [node(1) - 1, node(2)];
nodes{4} = [node(1), node(2) + 1];

% Compute the children for the 8-connectivity. The children for the
% 4-connectivity are a subset of these.
if (connectivity > 4)
    nodes{5} = [node(1) - 1, node(2) - 1];
    nodes{6} = [node(1) - 1, node(2) + 1];
    nodes{7} = [node(1) + 1, node(2) - 1];
    nodes{8} = [node(1) + 1, node(2) + 1];
end
...
costGradientDirection = atan2(gy,gx);
end
...
while any(path(end,:) ~= goalIdx)
    ...
    for i=1:numel(nodes)
        ...
        if ~(any(nodes{i} > mapSize) ||any(nodes{i} < 1)) && costs(nodes{i}(1), nodes{i}(2)) < minCost
            minCost = costs(nodes{i}(1), nodes{i}(2));
            minCostIdx = nodes{i};
