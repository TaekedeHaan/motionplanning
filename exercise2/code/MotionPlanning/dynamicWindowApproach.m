...
%         scores = parameters.alpha * headingTermsNormalized + ...
%                 parameters.beta * distTermsNormalized + ...
%                 parameters.gamma * velTermsNormalized;
...
% parameters.maxOmegaDot can be used for the maximum deceleration as well.
    ...
    maxVel = min(robotState.vel + parameters.maxAcc * parameters.timestep, parameters.maxVel);
    minVel = max(robotState.vel - parameters.maxAcc * parameters.timestep,  -parameters.maxVel);
    maxOmega = min(robotState.omega + parameters.maxOmegaDot * parameters.timestep, parameters.maxOmega);
    minOmega = max(robotState.omega - parameters.maxOmegaDot * parameters.timestep, -parameters.maxOmega);
...
% Invalid trajectories will get a NaN value assigned.
    ...
    if (stepsBeforeCollision >= nStepsBrake)
        ...
        if ~parameters.globalPlanningOn
            ...
            goalHeading = atan2(goalPosition.y  - robotStopPose.y, goalPosition.x - robotStopPose.x);
            %disp('local')
            headingTerm = abs(pi - abs(angleDiff(robotStopPose.heading, goalHeading)));
            ...
            if ~(any(finalTrajectoryIdx > maps.globalGradientMap.size) || any(finalTrajectoryIdx < 1))
                ...
                if isnan(gradientDirection) % that's the case at the goal cell, which is the only extremum in the cost function
                    ...
                    headingTerm = abs(pi - abs(angleDiff(robotStopPose.heading, gradientDirection)));
